...
export default class GetQuote extends Component{
    constructor(props){
        ...
        axios.post('http://localhost:4200/user/check-token', {token: localStorage.access_token}).then(res => {
            // console.log(res.data);
            // if(res.data.type === 'success') localStorage.setItem('access_token', res.data.token);
        });
    ...
    onSubmit(e){
        ...
        // console.log("Befor Storate")
        localStorage.setItem('getquote', JSON.stringify(getquote));
        // console.log("From Storage");
        // console.log(
        //     JSON.parse(localStorage.getItem('getquote'))
        // );
        this.setState({
            'part_one_done': true,
        });
        // this.state.part_one_done = true;
        // this.props.history.push('/user/homeownerinformation');
        // axios.post('http://localhost:4200/dashboard/getquote/', getquote).then(res => {
